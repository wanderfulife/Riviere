...
const downloadData = async () => {
  try {
    ...
    // Define base headers with proper order
    const headerOrder = [
      "ID_questionnaire",
      "ENQUETEUR",
      "DATE",
    ...
    props.questions.forEach((question) => {
      if (!headerOrder.includes(question.id)) {
        headerOrder.push(question.id);
        // Add precision field for questions that have it
        if (
          question.id === "Q14" ||
          question.options?.some((opt) => opt.text === "Autre")
        ) {
          headerOrder.push(`${question.id}_precision`);
        }
      }
    });
    ...
    const data = querySnapshot.docs.map((doc) => {
      ...
      Object.entries(docData).forEach(([key, value]) => {
        if (headerOrder.includes(key)) {
          ...
          if (key === "Q14" && Array.isArray(value)) {
            // For Q14, store the array of IDs as a comma-separated string
            row[key] = value.join(", ");
          } else {
            row[key] = value || "";
          }
    ...
    ws["!cols"] = headerOrder.map((key) => colWidths[key]);
    ...
    const date = new Date();
    const filename = `survey_data_${date.getFullYear()}-${String(
      date.getMonth() + 1

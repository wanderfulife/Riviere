        ...
        <div class="question-container" v-if="currentQuestion">
          ...
          <div
            v-if="
              ...
              !currentQuestion.usesCommuneSelector &&
              !currentQuestion.multipleChoice
            "
          >
            <div
              v-if="currentQuestion.id === 'Q3_precision'"
              class="list-container"
            >
              <select
                v-model="selectedOption"
                class="form-control"
                @change="selectAnswer({ id: selectedOption, next: 'Q4' })"
              >
          ...
          >
            ...
            <MultipleChoice
              ...
              @next="handleMultipleChoiceAnswer"
...
import MultipleChoice from "./widgets/MultipleChoice.vue";
...
// Firestore refs
const surveyCollectionRef = collection(db, "riviere");
const counterDocRef = doc(db, "counterriviere", "surveyCounter");
...
const handleCommuneSelection = () => {
  if (currentQuestion.value.usesCommuneSelector) {
    ...
    if (selectedValue && selectedValue.trim() !== "") {
      ...
      const baseQuestionId = questionId.replace("_precision", "");
...
const finishSurvey = async () => {
  ...
  const invalidFields = validateSurveyData(surveyData);
  if (invalidFields.length > 0) {
    console.error("Invalid fields found:", invalidFields);
    throw new Error(
      `Cannot save survey: Found invalid fields: ${invalidFields.join(", ")}`
    );
  }

  console.log(
    "Final validated survey data to be saved:",
...
const getNextId = async () => {
  ...
  return `riviere-${counter.toString().padStart(6, "0")}`;
};

const handleMultipleChoiceAnswer = ({ selectedOptions, precision }) => {
  if (currentQuestion.value) {
    const questionId = currentQuestion.value.id;
    answers.value[questionId] = selectedOptions;

    // If there's precision text (for "Autre" option)
    if (precision) {
      answers.value[`${questionId}_precision`] = precision;
    }

    // Find the next question based on the options
